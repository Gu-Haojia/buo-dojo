...
**[在线试玩](https://gu-haojia.github.io/buo-dojo/)** · **[问题反馈](https://github.com/Gu-Haojia/buo-dojo/issues)**
...
```sh
git clone https://github.com/Gu-Haojia/buo-dojo.git
cd buo-dojo
npm ci
npm run dev
```
...
点击结算框中的「Xにシェア」可预览本局分享卡。图片在浏览器内生成，使用本局的汉字顺序、实际成绩与随机人物 A／B；同一局重复打开或保存时保持同一张图。字体和素材加载后才导出 PNG，以确保生僻字和人物正常显示。

- 支持文件分享的手机可通过系统分享菜单选择 X，将图片与投稿文一起交给 X 应用。
- 其他环境使用「画像を保存してXへ」保存图片并打开预填文本的 X 投稿页，再由玩家手动附图。
- 提供单独保存图片、复制投稿文及受支持环境下的复制图片入口。图片生成失败时可以重试，也可以继续分享文字。

[X 网页投稿入口](https://docs.x.com/x-for-websites/post-button/overview)支持预填文本和链接，不能直接附加浏览器生成的本地图片。因此每局分享图作为图片附件使用，纯静态页面不会为每次结果生成可供 X 抓取的动态链接预览。系统分享目标由设备及已安装应用提供，参见 [Web Share API](https://www.w3.org/TR/web-share/)。
...
欢迎通过 [Issues](https://github.com/Gu-Haojia/buo-dojo/issues) 反馈问题或提出建议，也欢迎提交 Pull Request。
